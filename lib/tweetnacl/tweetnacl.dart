// adapted from https://github.com/GreenAppers/tweetnacl-dart
...
class Box {
  ...
  static KeyPair keyPair() {
    final KeyPair kp = KeyPair(publicKeyLength, secretKeyLength);
  ...
  static KeyPair keyPair_fromSecretKey(Uint8List secretKey) {
    final KeyPair kp = KeyPair(publicKeyLength, secretKeyLength);
    final Uint8List sk = kp.secretKey;
    final Uint8List pk = kp.publicKey;
...
class Signature {
  ...
  Uint8List open_len(
    ...
    // temp buffer
    final Uint8List tmp = Uint8List(smlen);
...
// 'expand 32-byte k'
  ...
  static int _crypto_stream_salsa20_xor(Uint8List c, int cpos, Uint8List m,
    ...
    final Uint8List z = Uint8List(16), x = Uint8List(64);
    int i;
    Int32 u;
    for (i = 0; i < 16; i++) {
      z[i] = 0;
  ...
  static void scalarmult(
    ...
    for (i = 255; i >= 0; --i) {
      final int b =
          ...
          ((Int32(s[(i / 8 + soff).toInt()]).shiftRightUnsigned(i & 7))
                  .toInt() &
              1);
...
// TBD... 64bits of n
  ...
  static int crypto_sign(Uint8List sm, int dummy /* *smlen not used*/,
    ...
    final Uint8List d = Uint8List(64), h = Uint8List(64), r = Uint8List(64);
...
// TBD optimizing ...
  ...
  static final jrandom = Random.secure();
...
class poly1305 {
  ...
  poly1305 blocks(Uint8List m, int mpos, int bytes) {
    final int hibit = this._fin != 0 ? 0 : (1 << 11);
